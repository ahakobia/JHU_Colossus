...
read.csv("data/raw-data/olympic2/Olympic_Athlete_Event_Results.csv") %>% 
  filter(grepl(pattern = "Summer Olympics", x = edition)) -> 
...
summer_olympic_athletes %>% 
  ...
  select(edition_id, athlete_id, country_noc) %>% 
  distinct() %>%  
...
summer_olympic_athletes %>% 
  filter(!isTeamSport == "False") %>% 
  select(edition_id, country_noc, sport, event) %>% 
  distinct() %>% 
  group_by(edition_id, country_noc) %>% 
  summarise(count = n()) -> 
  team_sport_events
...
write.csv(participants_country, file = file.path("data/country_athlete_count.csv"), row.names = FALSE)
...
write.csv(games_stats, file = file.path("data/games_stats.csv"), row.names = FALSE)
