...
interpolate_extrapolate <- function(year, gdppc, pop) {
  ...
  interpolated_pop <- na.approx(pop_z, xout = 1800:2022, rule = 2)
  interpolated_gdppc <- na.approx(gdppc_z, xout = 1800:2022, rule = 2)
...
mapping <- read.csv("data_kalyn/country_mapping.csv")
YRS <- pull(read.csv("data_kalyn/medal_count.csv"), year)
...
regional <- matrix(c( "American Samoa", "South and South East Asia",
                      ...
                      "Democratic People's Republic of Korea", "South and South East Asia",
                      ...
                      "West Indies Federation", "South and South East Asia",
                      "Australasia", "Western Offshoots",
                      "United Arab Republic", "Middle East and North Africa",
                      "Republic of Korea", "South and South East Asia",
                      "Soviet Union", "Western Europe",
                      "United Arab Republic", "Middle East and North Africa",
                      "Russian Olympic Committee", "Western Europe",
                      "Sudan", "Sub Saharan Africa",
                      "São Tomé and Príncipe", "South and South East Asia"), ncol = 2,  byrow = TRUE)
...
read_excel("data_kalyn/raw-data/mpd2023_web.xlsx", sheet =  "Full data") %>%
...
read_excel("data_kalyn/raw-data/mpd2023_web.xlsx", sheet =  "Regional data") %>%
...
write.csv(annual_GDP_values, file.path("data_kalyn", "annual_GDP.csv"), row.names = FALSE)
